    ...
    <body>
        ...
        <script>
            ...
            function sendMessage(event) {
                ...
                let send = {"room_id": "1", "name": input.value, "action": "connect"}
            ...
            function desconectar(event) {
                let input = document.getElementById("messageText")
                let send = {"room_id": "1", "name": input.value, "action": "disconnect"}
                ws.send(JSON.stringify(send))
            };
            function receive_res(event) {
                let input = document.getElementById("messageText")
                let send = {"room_id": "1", "name": input.value, "action": "receive_res", "is_correct": 1}
                ws.send(JSON.stringify(send))
            }
            function send_result(event) {
                let input = document.getElementById("messageText")
                let send = {"room_id": "1", "name": input.value, "action": "send_result",}
                ws.send(JSON.stringify(send)) 
            }
        </script>
    </body>
